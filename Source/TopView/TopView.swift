...
open class TopView: UIView {
  ...
  open lazy var flashButton: UIButton = { [unowned self] in
    let button = UIButton()
    button.setImage(AssetManager.getResourceImage("AUTO"), for: UIControl.State())
  ...
  open lazy var rotateCamera: UIButton = { [unowned self] in
    ...
    button.setImage(AssetManager.getResourceImage("cameraIcon"), for: UIControl.State())
  ...
  @objc func flashButtonDidPress(_ button: UIButton) {
    ...
    button.setImage(AssetManager.getResourceImage(newTitle), for: UIControl.State())

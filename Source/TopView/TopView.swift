...
open class TopView: UIView {
  ...
  open lazy var flashButton: UIButton = { [unowned self] in
    let button = UIButton()
    button.setImage(AssetManager.getResourceImage("AUTO"), for: UIControlState())
  ...
  open lazy var rotateCamera: UIButton = { [unowned self] in
    ...
    button.setImage(AssetManager.getResourceImage("cameraIcon"), for: UIControlState())
  ...
  @objc func flashButtonDidPress(_ button: UIButton) {
    ...
    button.setImage(AssetManager.getResourceImage(newTitle), for: UIControlState())
